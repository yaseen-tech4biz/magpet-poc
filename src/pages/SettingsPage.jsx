  ...
  return (
    ...
    >
      <div className="space-y-8 max-w-4xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Card>
            ...
            <input
              ...
              className="w-full font-mono text-xl font-bold text-[#143a72] bg-slate-50 border border-slate-300 rounded-lg p-3 focus:outline-hidden focus:border-[#143a72] focus:bg-white"
            />
          ...
          <Card>
            <div className="flex items-center justify-between mb-2">
              <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider font-heading">
            ...
            <div className="w-full font-mono text-base font-bold text-[#143a72] bg-slate-50 border border-slate-300 rounded-lg p-3 flex items-center justify-between">
              <span>Configured per machine</span>
              <span className="text-xs text-slate-500 font-normal">19.5g to 680g</span>
            </div>
            <p className="text-[11px] text-slate-500 mt-2">
        ...
        >
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-slate-200 text-[10px] font-mono font-bold text-slate-500 uppercase">
              ...
              <tbody className="divide-y divide-slate-100 font-mono">
                {[
                  ...
                  <tr key={m.code} className="hover:bg-slate-50/80 transition-colors">
                    <td className="py-2.5 px-3 font-bold text-[#143a72]">{m.code}</td>
                    <td className="py-2.5 px-3 font-sans text-slate-700">{m.vendor}</td>
                    <td className="py-2.5 px-3 text-slate-600">{m.cavities}</td>
                    <td className="py-2.5 px-3 font-sans text-slate-700">{m.product}</td>
                    <td className="py-2.5 px-3 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <input
                          ...
                          className="w-24 text-right font-mono font-bold text-slate-900 bg-white border border-slate-300 rounded px-2 py-1 focus:border-[#143a72] focus:outline-hidden"
                        />
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-900 tracking-tight font-heading mb-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Card>
              ...
              <div className="border-2 border-dashed border-slate-300 rounded-lg p-4 text-center hover:border-slate-400 transition-colors bg-slate-50/50">
                {logoUrl && (
                  <img
                    src={logoUrl}
                    alt="Logo preview"
                    className="h-10 mx-auto mb-2 object-contain bg-white p-1 rounded border border-slate-200"
                  />
                )}
                <input
                  type="file"
                  ...
                  className="text-xs text-slate-500 file:mr-3 file:py-1 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-[#143a72] file:text-white hover:file:bg-[#0c2347] cursor-pointer"
                />
              </div>
              <p className="text-[11px] text-slate-500 mt-2">
            ...
            <Card>
              ...
              <p className="text-[11px] text-slate-500 mt-3">
                Default: Deep Magpet Navy <code className="font-mono text-[#143a72] font-semibold">#143A72</code> from magnumgroup.in.
              </p>
            </Card>
          </div>
        </div>
        ...
        <Card title="What These Numbers Mean Right Now (Live Recomputed)">
          <div className="divide-y divide-slate-100 text-xs">
            <div className="py-3 flex items-center justify-between">
              <span className="text-slate-600">One hour of unplanned rPET stoppage (resin value)</span>
              <span className="font-mono font-bold text-slate-900 text-sm">{formatResinHeld(1)}</span>
            </div>
            <div className="py-3 flex items-center justify-between">
              <span className="text-slate-600">Last night's EX-02 trip (3.5 hours)</span>
              <span className="font-mono font-bold text-rose-600 text-sm">{formatResinHeld(3.5)}</span>
            </div>
            <div className="py-3 flex items-center justify-between">
              <span className="text-slate-600">EX-02 downtime this quarter (34.5 hours)</span>
              <span className="font-mono font-bold text-rose-600 text-sm">{formatResinHeld(34.5)}</span>
            </div>
            <div className="py-3 flex items-center justify-between">
              <span className="text-slate-600">Preform rejection bill (month to date)</span>
              <span className="font-mono font-bold text-amber-600 text-sm">{formatRejBill()}</span>
            </div>
            <div className="py-3 flex items-center justify-between">
              <span className="text-slate-600">Preform rejection bill run rate (annualised)</span>
              <span className="font-mono font-bold text-rose-700 text-sm">{formatRejBillAnnual()}</span>
            </div>
          </div>
        </Card>
