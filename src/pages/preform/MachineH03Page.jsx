        ...
        <div className="flex items-center justify-between gap-4 w-full normal-case tracking-normal">
          <div className="flex items-center gap-2 min-w-0 truncate">
            <span className="text-xs font-mono text-slate-700 dark:text-slate-300 whitespace-nowrap">
              <strong className="text-slate-900 dark:text-white font-bold">Hooghly Unit 1</strong> · Machine Drilldown · {currentMachineObj.name.split('·')[0]}
            </span>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            >
              {machines.map((m) => (
                <option key={m.id} value={m.id} className="dark:bg-slate-800 dark:text-slate-200">
                  {m.id} · {m.name.split('·')[0]}
                </option>
              ))}
            </select>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          >
            ...
            {/* Defect Legend */}
            <div className="flex flex-wrap gap-4 mt-4 text-xs text-slate-600 dark:text-slate-300 border-t border-slate-100 dark:border-slate-800 pt-3">
              {defectNames.map((d, i) => (
                <div key={d} className="flex items-center gap-1.5">
                  ...
                  />
                  <span className={selectedMachine === 'H-03' && i === 3 ? 'font-bold text-purple-900 dark:text-purple-300' : ''}>
                    {d}
                  </span>
                </div>
              ))}
        ...
        <div className="lg:col-span-5 space-y-6">
            ...
            rightElement={
              <span className={`font-mono text-[10px] px-2 py-0.5 rounded font-semibold uppercase tracking-wider ${
                currentMachineObj.isHusky 
                  ? 'text-rose-600 dark:text-rose-300 bg-rose-50 dark:bg-rose-950/50 border border-rose-200 dark:border-rose-900' 
          ...
          >
            <div className="grid grid-cols-3 gap-2.5 my-1">
              <div className="p-2.5 bg-slate-50 dark:bg-slate-800/60 rounded-lg border border-slate-200 dark:border-slate-700/80 text-center">
                <div className="text-[10px] font-mono text-slate-400 dark:text-slate-500 uppercase">Nominal</div>
                <div className="text-lg font-bold font-mono text-emerald-700 dark:text-emerald-400">{cavityStats.nominal}</div>
                <div className="text-[10px] text-slate-500 dark:text-slate-400">±0.15g</div>
              </div>
              <div className="p-2.5 bg-slate-50 dark:bg-slate-800/60 rounded-lg border border-slate-200 dark:border-slate-700/80 text-center">
                <div className="text-[10px] font-mono text-slate-400 dark:text-slate-500 uppercase">Drift</div>
                <div className="text-lg font-bold font-mono text-amber-600 dark:text-amber-400">{cavityStats.warning}</div>
                <div className="text-[10px] text-slate-500 dark:text-slate-400">±0.25g</div>
              </div>
              <div className="p-2.5 bg-slate-50 dark:bg-slate-800/60 rounded-lg border border-slate-200 dark:border-slate-700/80 text-center">
                <div className="text-[10px] font-mono text-slate-400 dark:text-slate-500 uppercase">Critical</div>
                <div className="text-lg font-bold font-mono text-rose-600 dark:text-rose-400">{cavityStats.critical}</div>
                <div className="text-[10px] text-slate-500 dark:text-slate-400">&gt;0.25g</div>
              </div>
            </div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800">
              <Link
                ...
                className="inline-flex items-center justify-center w-full bg-[#143a72] hover:bg-[#0c2347] dark:bg-blue-600 dark:hover:bg-blue-500 text-white font-semibold text-xs py-2.5 px-4 rounded-lg shadow-2xs transition-colors"
              >
